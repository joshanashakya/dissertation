...
l_1[i] < l_2[j]: output_list.append(l_1[i])
...
output_list = []
char = ")"
for test_case in test_cases:        n = int(input())
l = input()
count = 0
for i in range(n):            if
l[len(l) - 1 - i] == char: count += 1 else:                break
if count > n - count:
    ans = "Yes"        else:
    ans = "No"
output_list.append(ans)
for i in output_list:        print(
    i)   # string = list_str(sorted_l, " ")    #     output_list.append(string)    # return list_str(output_list, "\n")   (main_function()) 
