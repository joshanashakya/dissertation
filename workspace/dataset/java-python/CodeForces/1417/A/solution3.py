...
return l   # k = [ 2,4,3,2,1,2,3,4,5,6,5,4,3,2]# quicksort(k)# print(k)  def collector_recursive(l):    output_list = []    def colec(l, start, end, cl):        if start > end:            return cl        else:            cl.append(f"{l[start][1]} {l[end][1]}")            return colec(l, start + 1, end - 1, cl)    return colec(l, 0, len(l) - 1, output_list)  def find_min(l):    mini = l[0]    ind = 0    for i in range(len(l)):        if mini > l[i]:            mini = l[i]            ind = i    return ind     def main_function():    output_list  = []    test_cases = range(int(input()))    for test_case in test_cases:        n, k = str_list(input(), " ")        l = str_list(input(), " ")        ind_mini = find_min(l)        count = 0        for i in range(len(l)):            if not i == ind_mini:                dif = k - l[i]                count += dif // l[ind_mini]        output_list.append(count)    return list_str(output_list, "\n")  print(main_function())
