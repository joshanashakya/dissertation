...
public class BoboniuChatsWithDu {
	...
	public static void main(String[] args) throws IOException {
		...
		long ans = arr2.get(arr2.size() - 1) + (arr1.size() > 1 ? arr1.get(1) : 0);
		...
		for (int i = 1; i < arr1.size(); i++) {
			...
			long c = n;
			if (a * b + 1l <= c) {
				long dayleft = c - (a * b + 1l);
				if (dayleft >= (long) (arr2.size() - 1)) {
					dayleft = (long) (arr2.size() - 1);
				}
				ans = Math.max(ans, arr1.get(i) + arr2.get((int) dayleft));
			} else
				break;
		}
		p.println(ans);
